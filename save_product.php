...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['product_id']) && !empty($_POST['product_id'])) {
        ...
        $query = "UPDATE products SET name = ?, description = ?, categories = ?, branding_options = ?, logo_hex_color = ?";

        $params = [$name, $description, $category_id, $branding_option, $logo_hex_color];
        $types = "sssss";
        ...
        // Insert new product
        $stmt = $conn->prepare("INSERT INTO products (name, description, featured_image, categories, logo_positions, branding_options, logo_styles, logo_path, logo_hex_color) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if ($stmt === false) {
            die("Error: Failed to prepare the query - " . $conn->error);
        }

        $stmt->bind_param('sssssssss', $name, $description, $imagePath, $category_id, $logo_positions, $branding_option, $logo_styles, $logoPath, $logo_hex_color);
    }
